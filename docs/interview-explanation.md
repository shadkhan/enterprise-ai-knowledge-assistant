...
The implementation is intentionally a skeleton. It uses mock data and mock LLM responses so the architecture is easy to understand, but each module is shaped like a production component that can be replaced with real connectors, pgvector search, enterprise identity, and commercial or local LLM providers.
...
The ingestion module accepts documents, extracts metadata, and chunks text. Today it uses an in-memory store; production would replace this with object storage for raw files, PostgreSQL metadata, and pgvector embeddings. Ingestion should run asynchronously through Celery or another queue because parsing, OCR, embedding, and permission sync are slow and failure-prone.
